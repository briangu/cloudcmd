...
class Remove extends Command {
  ...
  def doCommand(adapter: IndexedContentAddressableStorage) {
    ...
    if (adapter == CloudServices.BlockStorage) {
      ...
      if (_removeBlockHashes) {
        System.err.println("removing meta and file data for %d files from adapter: %s".format(jsonFileMetaDataArray.length(), adapter.asInstanceOf[DirectAdapter].getSignature))
      } else {
        System.err.println("removing meta data for %d files from adapter: %s".format(jsonFileMetaDataArray.length(), adapter.asInstanceOf[DirectAdapter].getSignature))
      }
    }
